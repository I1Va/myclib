/* bot.c - freestanding, uses mini‑libc */
...
static int try_move(BotState *st, int8_t dx, int8_t dy, const char *why) {
    ...
    if (st->have_pos && wall_known(st, nx, ny)) return 0;
...
static void do_random_action(BotState *st) {
    ...
    if (st->have_pos && st->visible_count > 0) {
        size_t best = 0;
        int best_dist = long_dist(st->self_x, st->self_y,
                                  st->visible[0].x, st->visible[0].y);
        for (size_t i = 1; i < st->visible_count; i++) {
            int d = long_dist(st->self_x, st->self_y,
                              st->visible[i].x, st->visible[i].y);
            if (d < best_dist) {
                best = i;
                best_dist = d;
            }
    ...
    if (legal_count == 0) {
        fprintf_stderr("boxed in by known walls; skipping tick\n");
        clear_tick_state(st);
        return;
    }
...
void handle_person_message(BotState *st, const char *type,
    ...
    if (strcmp(type, "hp") == 0) {
        ...
        fprintf_stderr("hp = %d\n", st->hp);
        if (st->hp <= 0) {
            fprintf_stderr("dead; closing connection\n");
            st->alive = 0;
        }
        return;
    }
    ...
    if (strcmp(type, "sees") == 0) {
        ...
        fprintf_stderr("sees id=%u at (%d, %d)\n", who, x, y);
        return;
    }
    ...
    if (strcmp(type, "wall") == 0) {
        ...
        int32_t y = rd_i32_le(payload + 4);
        add_wall(st, x, y);
        fprintf_stderr("wall at (%d, %d)\n", x, y);
        return;
    }
}

void handle_srv_message(BotState *st, const char *type,
                        const uint8_t *payload, uint16_t len) {
    (void)st;

    if (strcmp(type, "hasPref") == 0) {
        if (len != 8) {
            fprintf_stderr("bad srv:hasPref len=%u\n", (unsigned)len);
            return;
        }
        char pref[9];
        rd_char64(pref, payload);
        fprintf_stderr("srv:hasPref pref='%s'\n", pref);
        return;
    }
    ...
    if (strcmp(type, "name") == 0) {
        ...
        if (rd_pan_string(payload, len, name, sizeof(name)) < 0) {
            fprintf_stderr("bad srv:name len=%u\n", (unsigned)len);
            return;
        }
        fprintf_stderr("srv:name name='%s'\n", name);
        return;
    }

    if (strcmp(type, "id") == 0) {
        if (len != 4) {
            fprintf_stderr("bad srv:id len=%u\n", (unsigned)len);
            return;
        }
        uint32_t client_id = rd_u32_le(payload);
        fprintf_stderr("srv:id clientId=%u\n", client_id);
        return;
    }

    if (strcmp(type, "level") == 0) {
        if (len != 8) {
            fprintf_stderr("bad srv:level len=%u\n", (unsigned)len);
            return;
        }
        char level[9];
        rd_char64(level, payload);
        fprintf_stderr("srv:level level='%s'\n", level);
        return;
    }

    if (strcmp(type, "r.setLvl") == 0) {
        if (len != 5) {
            fprintf_stderr("bad srv:r.setLvl len=%u\n", (unsigned)len);
            return;
        }
        uint32_t msg = rd_u32_le(payload + 0);
        int ok = rd_bool_u8(payload + 4);
        fprintf_stderr("srv:r.setLvl msg=%u ok=%d\n", msg, ok);
        return;
    }

    fprintf_stderr("ignoring unknown srv:%s len=%u\n", type, (unsigned)len);
}